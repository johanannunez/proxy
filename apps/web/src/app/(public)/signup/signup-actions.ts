...
import { isValidOrgSlug } from "@/lib/organizations/slug";
import { PLAN_FEATURES, PLAN_LIMITS } from "@/lib/organizations/features";
 ...
 * Format and reserved-name rules live in @/lib/organizations/slug so the
 * client can mirror them instantly; this action is the authoritative check
 * against existing organizations.
 */
...
/**
 * Final submit of the signup flow: creates the auth user, the organization,
 ...
 * via authCookieDomain). Organization writes go through the service client
 * because organizations tables only allow service-role writes under RLS.
 */
export async function createOrganization(params: {
  ...
  if (orgError || !org) {
    ...
    console.error("[signup] org insert failed:", orgError?.message);
  ...
  //    fail, roll the org back (cascade removes the partial rows) so a retry
  //    can reuse the slug.
  const { error: memberError } = await db
    .from("organization_members")
    .insert({ org_id: org.id, profile_id: user.id, role: "org_owner" });

  const { error: brandingError } = memberError
    ? { error: memberError }
    : await db.from("organization_branding").insert({ org_id: org.id });
  ...
  if (seedError) {
    console.error("[signup] org seed failed:", seedError.message);
...
export async function createStripeSubscription(params: {
  ...
  try {
    ...
    // keep organizations.plan_tier in sync for the rest of the lifecycle.
    ...
    if (updateError) {
      console.error(
        "[signup] failed to store Stripe ids on org:",
        updateError.message,
      );
    }
