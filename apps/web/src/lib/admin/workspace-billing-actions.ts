...
const ScheduleInput = WorkspaceInput.extend({
  ...
  lines: z.array(BillingLineInput).min(1, "Add at least one line item.").max(20),
});
...
function lineTotalCents(line: z.infer<typeof BillingLineInput>): number {
  return Math.max(
    0,
...
function lineCostCents(line: z.infer<typeof BillingLineInput>): number {
  return Math.max(
    0,
...
export async function createWorkspaceBillingScheduleAction(
  raw: unknown,
  ...
  try {
    ...
    if (scheduleError || !schedule) {
      throw new Error(scheduleError?.message ?? "Could not create billing schedule.");
    }
...
export async function approveWorkspaceBillingInvoiceAction(
  raw: unknown,
): Promise<MutationResult> {
  ...
  try {
    ...
    const stripeInvoice = await stripe.invoices.create({
      ...
      metadata: {
        parcel_workspace_id: invoice.workspace_id,
        parcel_billing_invoice_id: invoice.id,
        parcel_billing_profile_id: invoice.billing_profile_id,
      },
    });
    ...
    for (const line of lines) {
      await stripe.invoiceItems.create({
        ...
        metadata: {
          parcel_workspace_id: invoice.workspace_id,
          parcel_billing_invoice_id: invoice.id,
          parcel_billing_invoice_line_id: line.id,
        },
      });
    }
...
export async function voidWorkspaceBillingInvoiceAction(
  raw: unknown,
): Promise<MutationResult> {
  const parsed = InvoiceActionInput.safeParse(raw);
  if (!parsed.success) {
